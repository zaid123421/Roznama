...
export default function Header() {
  ...
  return (
    <div className="container px-[25px] m-auto flex justify-between items-center relative">
      <i
        onClick={handleClick}
        className="lg:hidden hover:text-green-600 duration-300 cursor-pointer text-2xl fa-solid fa-bars-staggered"
      ></i>
      {isOpened && (
      ...
      <ul className="lg:flex min-[0px]:hidden max-[1024px]">
        <li className="mr-5 text-2xl hover:text-green-600 duration-300">
          <NavLink
            to="/reminders"
            className={({ isActive }) =>
              isActive ? "text-green-600 underline underline-offset-[15px]" : ""
            }
          >
            التذكيرات
          </NavLink>
      ...
      <img className="logo w-[175px]" alt="logo" src={logo} />
    </div>
  );
}
